    ...
    this.pokemonService.getTypes().subscribe(
      (types: any) => {
        for (let i = 0; i < types.results.length; i++) {
          ...
          this.pokemonService.getPokemonsPerType(types.results[i].url.substring('https://pokeapi.co/api/v2/type/'.length))
            .subscribe(
              (typeData: any) => {
                this.typesLoaded++;
    ...
    this.regionService.getRegions().subscribe(
      (regions: any) => {
        ...
        for (let i = 0; i < regions.results.length; i++) {
          ...
          this.regionService.getRegion(regions.results[i].url.substring('https://pokeapi.co/api/v2/region/'.length))
            .subscribe(
              (regionData: any) => {
                this.regionsLoaded++;
